...
\node[below, inner sep=0] at ([yshift=-1.6cm]l1.center) {\tiny$k=127$};
\node[rectangle,draw=none,inner sep=0,below] (l2) at ([xshift=5.5cm]Cuba) {\includegraphics[width=3cm]{183.png}};
\node[below, inner sep=0] at ([yshift=-1.6cm]l2.center) {\tiny$k=183$};

\node[rectangle,right,align=left] at ([yshift=-4.5cm]Cuba) {Zeros of the  $k$-th\\ 
...
\node[below, inner sep=0] at ([yshift=-1.6cm]r1.center) {\tiny$n,m=5,11$};
\node[rectangle,draw=none,inner sep=0,below] (r2) at ([xshift=-5.5cm]TurksCaicos) {\includegraphics[width=3cm]{713.png}};
\node[below, inner sep=0] at ([yshift=-1.6cm]r2.center) {\tiny$n,m=7,13$};

\node[rectangle,left,align=right] at ([yshift=-4.5cm]TurksCaicos) {Zeros of \\ 
...
\node[rectangle, draw=none, red!50!yellow!20] at (0,-2) {\LARGE\it Happy New Math Year 2021};
...
Chladni figures
http://w3.phys.nthu.edu.tw/~gplab/file/07%20Standing%20Waves/chladni%20patterns%20(US).pdf

nodal domain theorem: try discrete version, fix center
